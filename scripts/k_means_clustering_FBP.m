...
for o=1:size(num_theta, 2)
    for k=1:size(num_clusters, 2)
        ...
        for i=1:30
            reconstructed_image = iradon(projections, better_theta, output_size);

            % Reconstruct the image.
            reconstructed_image(reconstructed_image < 0) = 0;
            
            % The error we optimise.
            [projections_estimated, svector] = ...
                radon(reconstructed_image, better_theta);
            function_error = norm(projections_estimated - projections);
