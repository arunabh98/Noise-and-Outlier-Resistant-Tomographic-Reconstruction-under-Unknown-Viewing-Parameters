...
for candidate_index = 2:numkeep %candidate for theta2
    ...
    temp = thetaspredicted(2);                                  %reversing the swap
    thetaspredicted(2) = thetaspredicted(candidate_index);      %reversing the swap
    thetaspredicted(candidate_index) = temp;                    %reversing the swap
...
% dlmwrite('data.csv', [thetaskeep', thetaerrorvector, (sum(noise))', IMvectors(:,:,1), IMvectors(:,:,2), IMvectors(:,:,3)])
 dlmwrite('thetasdata.csv', thetaspredicted)
% format
