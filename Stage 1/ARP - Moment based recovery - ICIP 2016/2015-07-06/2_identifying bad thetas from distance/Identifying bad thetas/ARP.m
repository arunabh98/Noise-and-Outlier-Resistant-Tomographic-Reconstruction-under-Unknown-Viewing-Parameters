...
noisydistmatrix = zeros(30);
for i = 1:30
    for j = 1:30 
        noisydistmatrix(i,j) = mydist(Pgiven(:,i), Pgiven(:,j));
    end
end
[bestthetaspredictedsorted, order] = sort(bestthetaspredicted);
noisydistmatrix = noisydistmatrix(order, :);
noisydistmatrix = noisydistmatrix(:, order);

dlmwrite('noisydistances.csv', [bestthetaspredictedsorted'; noisydistmatrix]);
format shortG
[bestthetaspredicted, expectedanswer_original, abs(bestthetaspredicted - expectedanswer_original) <=5 ]
[bestthetaspredictedsorted, expectedanswer_original(order), abs(bestthetaspredictedsorted - expectedanswer_original(order)) <=5 ]
sum(1 - (abs(bestthetaspredictedsorted - expectedanswer_original(order)) <=5))
