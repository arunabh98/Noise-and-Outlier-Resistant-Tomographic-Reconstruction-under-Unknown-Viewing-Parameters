...
while not_done
    ...
    keep(to_drop) = [];
    numkeep = numkeep - length(to_drop);
    ...
    for row = 1:length(thetaspredicted)
        thetaspredicted(row, :) = thetaspredicted(row, :) * mode(sign(thetaspredicted(row, :)));
    end
    ...
    param = 1;
    for thetareference_index = 1:param:numkeep


        %need to rotate for median filtering!
        distvector = noisydistmatrix(thetareference_index ,:)';

        medianfilter = rotatedmediansmooth(distvector, thetareference_index, 3);

%         %figure()
%         plot(bestthetaspredictedsorted_median, distvector, 'ro-', ...
%              bestthetaspredictedsorted_median, rotatedmediansmooth(distvector, thetareference_index,3) , 'k.-', ...
%              bestthetaspredictedsorted_median, smooth(distvector, 'sgolay', 3) , 'm.-');
%         
% 
...
%         legend('By computed angles', 'Median filter(5)', 'sgolay (3)', 'Incorrectly predicted angles');


        curve = noisydistmatrix(thetareference_index ,:)';
        smoothcurve = medianfilter; %smooth(distvector, 'sgolay', 3);
        smoothingerror = sum(abs(curve - smoothcurve));

        incorrect_indicator = zeros(numkeep, 1);
        for removalcandidate = 1:numkeep
            xcurve = curve;
            xcurve(removalcandidate) = [];

            xthetareference_index = thetareference_index;
            if removalcandidate < xthetareference_index % Note: if they are equal, we are making the next one the reference candidate.
                xthetareference_index = xthetareference_index - 1;
            end

            xsmoothcurve = rotatedmediansmooth(xcurve, xthetareference_index, 3);
            %xsmoothcurve = smooth(xcurve, 'sgolay', 3);
            
            xsmoothingerror = sum(abs(xcurve - xsmoothcurve));

            incorrect_indicator(removalcandidate) = ( (smoothingerror - xsmoothingerror) / smoothingerror ) > 0.1 ;
    ...
    vote     = mode(predicted_as_bad(:,1:param:numkeep)')';
    m1edian  = median(predicted_as_bad(:,1:param:numkeep)')';
    m1ean    = mean(predicted_as_bad(:,1:param:numkeep)')';
    [expectedanswer_original_ordered, bestthetaspredictedsorted_median, not(mediancorrect), vote, m1edian, m1ean, predicted_as_bad(:,1:param:numkeep)]
    

    to_drop = find(m1edian > 0)
    to_drop_actual = [];
    for t = to_drop'
        to_drop_actual = [to_drop_actual; find(expectedanswer_original == expectedanswer_original_ordered(t))];
        %Actual coordinate in terms of Pgiven
    end
     
    to_drop = to_drop_actual;
    not_done = ~isempty(to_drop);
%     if iter_count == 2
%         break
%     end
     pause(1);
